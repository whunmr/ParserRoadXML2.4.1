              ...
              double    x)
{
	...
	if (DeltaX!=0)
		return y1 + (x - x1) * (y2 - y1) / DeltaX;
	else
        return (y2 + y1) *0.5;
}

Vector2 stk::intlin(const Vector2 &P1, const Vector2 &P2, double x1, double x2, double x)
{
	return Vector2(
		intlin(P1.x,P2.x,x1,x2,x),
		intlin(P1.y,P2.y,x1,x2,x)
	);
}

Vector3 stk::intlin(const Vector3 &P1, const Vector3 &P2, double x1, double x2, double x)
{
	return Vector3(
		intlin(P1.x,P2.x,x1,x2,x),
		intlin(P1.y,P2.y,x1,x2,x),
		intlin(P1.z,P2.z,x1,x2,x)
	);
}
